...
</li>`).join("");m.insertAdjacentHTML("beforeend",t),q.refresh()}function $(){m.innerHTML=""}function M(){p.classList.add("visible")}function g(){p.classList.remove("visible")}function b(){y.classList.remove("hidden")}function l(){y.classList.add("hidden")}const E=document.querySelector(".form"),O=document.querySelector(".load-more");let a=1,i="",d=0;E.addEventListener("submit",async o=>{o.preventDefault();debugger;if(i=o.target.elements["search-text"].value.trim(),!!i){a=1,$(),l(),M();try{const t=await f(i,a);if(d=t.totalHits,t.hits.length===0){n.error({message:"Sorry, no images found."});return}h(t.hits),b(),a*15>=d&&(l(),n.info({message:"We're sorry, but you've reached the end of search results."}))}catch{n.error({message:"Something went wrong"})}finally{g()}}});O.addEventListener("click",async()=>{debugger;a+=1,l();try{const o=await f(i,a);h(o.hits);const t=document.querySelector(".gallery").firstElementChild.getBoundingClientRect();window.scrollBy({top:t.height*2,behavior:"smooth"}),a*15<d?b():n.info({message:"We're sorry, but you've reached the end of search results."})}catch{n.error({message:"Something went wrong"})}finally{g()}});
